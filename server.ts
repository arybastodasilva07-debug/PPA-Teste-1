...
async function startServer() {
  ...
  db.exec(`
    ...
    CREATE TABLE IF NOT EXISTS teacher_classes (
      id INTEGER PRIMARY KEY AUTOINCREMENT,
      user_id INTEGER,
      name TEXT, -- e.g., "Turma A"
      classe TEXT, -- e.g., "1ª Classe"
      subject TEXT, -- e.g., "Matemática"
      period TEXT, -- e.g., "Manhã"
      FOREIGN KEY(user_id) REFERENCES users(id)
    );
  `);
  ...
  io.on("connection", (socket) => {
    ...
    socket.on("send_message", (data) => {
      ...
      const user = db.prepare("SELECT professor_nome, foto_url FROM users WHERE id = ?").get(userId) as any;
      const result = db.prepare("INSERT INTO chat_messages (user_id, message) VALUES (?, ?)").run(userId, message);
      
      const newMessage = {
        id: result.lastInsertRowid,
        user_id: userId,
        message,
        professor_nome: user?.professor_nome || "Professor",
  ...
  app.post("/api/auth/login", (req, res) => {
    ...
    if (user) {
      ...
      if (!user.is_admin && user.data_expiracao) {
        ...
        if (now > expiry) {
          return res.status(403).json({ error: "Sua assinatura expirou. Entre em contato pelo WhatsApp: 948 298 246 / 954 458 413 para renovar seu plano." });
        }
      }
      res.json({ user });
    } else {
  ...
  app.post("/api/auth/register", (req, res) => {
    const { email, telefone, escola, professor_nome, provincia, municipio, plano_tipo, password } = req.body;
    
    if (!password || password.length < 4) {
      return res.status(400).json({ error: "A senha deve ter pelo menos 4 caracteres." });
    }

    // Determine trial days based on selected plan
    let trialDays = 7;
    if (plano_tipo === 'Teste Grátis (14 Dias)') trialDays = 14;
    if (plano_tipo === 'Teste Grátis (30 Dias)') trialDays = 30;
    
    const dataAtivacao = new Date();
    const dataExpiracao = new Date();
    dataExpiracao.setDate(dataExpiracao.getDate() + trialDays);

    try {
      const result = db.prepare("INSERT INTO users (email, telefone, password, status, escola, professor_nome, provincia, municipio, plano_tipo, planos_consumidos, data_ativacao, data_expiracao) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)").run(
        email || null,
        telefone || null,
        password, // User-provided password
        'Ativo', // Automatically activate
        escola,
        professor_nome,
        provincia,
        municipio,
        plano_tipo || 'Teste Grátis (7 Dias)',
        0,
        dataAtivacao.toISOString(),
  ...
  app.get("/api/library/files", (req, res) => {
    ...
    console.log(`Scanning library files in: ${libraryPath}`);
  ...
  const storage = multer.diskStorage({
    destination: (req, file, cb) => {
      // Try to get folder from body, query, or headers to ensure we have it
      const folder = req.body.folder || req.query.folder || req.headers['x-folder'] || "";
      const decodedFolder = decodeURIComponent(folder.toString());
      const dest = path.join(libraryPath, decodedFolder);
      
      console.log(`Uploading file: ${file.originalname} to folder: ${decodedFolder} (Full path: ${dest})`);
      
      if (!fs.existsSync(dest)) fs.mkdirSync(dest, { recursive: true });
      cb(null, dest);
    },
    filename: (req, file, cb) => {
      // Ensure filename is safe and unique if needed, or just use original
      // Using original name as per requirement, but handling potential encoding issues
  ...
  app.post("/api/admin/library/upload", upload.single("file"), (req, res) => {
    ...
    // Notify news
    try {
      db.prepare("INSERT INTO news (title, content, category, source) VALUES (?, ?, ?, ?)").run(
        `Novo Documento: ${fileName}`,
        `Um novo documento foi adicionado à biblioteca na pasta: ${folder || 'Raiz'}.`,
        'Aviso',
        'Sistema de Biblioteca'
      );
    } catch (e) {
      console.error("Error creating news for library upload:", e);
    }
  ...
  app.get("/api/library/download/:filepath", (req, res) => {
    const filePath = path.join(libraryPath, req.params.filepath);
    if (fs.existsSync(filePath)) {
  ...
  app.post("/api/ai/sync-news", async (req, res) => {
    try {
      ...
      res.status(500).json({ error: "Erro ao sincronizar notícias com IA" });
    }
  });
  ...
  app.post("/api/admin/update-user", (req, res) => {
    const { id, data_expiracao, plano_tipo, limite_planos, status, professor_nome, escola } = req.body;
    ...
    db.prepare(`
      UPDATE users 
      SET data_expiracao = ?, plano_tipo = ?, limite_planos = ?, status = ?, data_ativacao = ?, professor_nome = ?, escola = ?
      WHERE id = ?
    `).run(
      data_expiracao || user.data_expiracao, 
      plano_tipo || user.plano_tipo, 
      limite_planos !== undefined ? limite_planos : user.limite_planos, 
      status || user.status, 
      isNowAtivo && !user.data_ativacao ? new Date().toISOString() : user.data_ativacao,
  ...
  app.post("/api/admin/curriculum/add", (req, res) => {
    ...
    try {
      if (tema) {
        existing = db.prepare("SELECT * FROM curriculum WHERE classe = ? AND disciplina = ? AND tema = ? AND (subtema = ? OR (subtema IS NULL AND ? = ''))").get(classe, disciplina, tema, st, st) as any;
      } else {
        existing = db.prepare("SELECT * FROM curriculum WHERE classe = ? AND disciplina = ? AND tema IS NULL AND (subtema = ? OR (subtema IS NULL AND ? = ''))").get(classe, disciplina, st, st) as any;
      }
      ...
      console.error("Error adding curriculum:", e);
      res.status(500).json({ error: "Erro ao adicionar currículo" });
    }
  });
  ...
  app.post("/api/admin/curriculum/edit", (req, res) => {
    ...
    console.log(`Editing curriculum: type=${type}, oldData=${JSON.stringify(oldData)}, newData=${JSON.stringify(newData)}`);
    ...
    try {
      if (type === 'disciplina') {
        ...
        if (tema) {
          ...
          db.prepare("UPDATE curriculum SET tema = ? WHERE classe = ? AND disciplina = ? AND tema IS NULL").run(newData.name, classe, disciplina);
        }
      } else if (type === 'subtema') {
        if (tema) {
          db.prepare("UPDATE curriculum SET subtema = ? WHERE classe = ? AND disciplina = ? AND tema = ? AND (subtema = ? OR (subtema IS NULL AND ? = ''))").run(newData.name, classe, disciplina, tema, st, st);
        } else {
          db.prepare("UPDATE curriculum SET subtema = ? WHERE classe = ? AND disciplina = ? AND tema IS NULL AND (subtema = ? OR (subtema IS NULL AND ? = ''))").run(newData.name, classe, disciplina, st, st);
        }
        ...
        if (existing) {
          ...
          if (idx !== -1) {
            ...
            console.log("Sumario not found in array:", sumario, sumarios);
          }
        } else {
          console.log("Existing row not found for sumario edit");
        }
      }
      res.json({ success: true });
    } catch (e) {
      console.error("Error editing curriculum:", e);
      res.status(500).json({ error: "Erro ao editar currículo" });
    }
  });
  ...
  app.post("/api/admin/curriculum/remove", (req, res) => {
    ...
    try {
      if (sumario) {
        ...
        if (tema) {
          db.prepare("DELETE FROM curriculum WHERE classe = ? AND disciplina = ? AND tema = ? AND (subtema = ? OR (subtema IS NULL AND ? = ''))").run(classe, disciplina, tema, st, st);
        } else {
          db.prepare("DELETE FROM curriculum WHERE classe = ? AND disciplina = ? AND tema IS NULL AND (subtema = ? OR (subtema IS NULL AND ? = ''))").run(classe, disciplina, st, st);
        }
      } else if (tema) {
      ...
      console.error("Error removing curriculum:", e);
      res.status(500).json({ error: "Erro ao remover currículo" });
    }
  });

  app.post("/api/admin/update-settings", (req, res) => {
    const { escola, professor, provincia, municipio, smtp_host, smtp_port, smtp_secure, smtp_user, smtp_pass, admin_email } = req.body;
    
    if (escola !== undefined) db.prepare("INSERT OR REPLACE INTO settings (key, value) VALUES ('default_escola', ?)").run(escola);
    if (professor !== undefined) db.prepare("INSERT OR REPLACE INTO settings (key, value) VALUES ('default_professor', ?)").run(professor);
    if (provincia !== undefined) db.prepare("INSERT OR REPLACE INTO settings (key, value) VALUES ('default_provincia', ?)").run(provincia);
    if (municipio !== undefined) db.prepare("INSERT OR REPLACE INTO settings (key, value) VALUES ('default_municipio', ?)").run(municipio);
    
    if (smtp_host !== undefined) db.prepare("INSERT OR REPLACE INTO settings (key, value) VALUES ('smtp_host', ?)").run(smtp_host);
    if (smtp_port !== undefined) db.prepare("INSERT OR REPLACE INTO settings (key, value) VALUES ('smtp_port', ?)").run(smtp_port);
    if (smtp_secure !== undefined) db.prepare("INSERT OR REPLACE INTO settings (key, value) VALUES ('smtp_secure', ?)").run(smtp_secure ? 'true' : 'false');
    if (smtp_user !== undefined) db.prepare("INSERT OR REPLACE INTO settings (key, value) VALUES ('smtp_user', ?)").run(smtp_user);
    if (smtp_pass !== undefined) db.prepare("INSERT OR REPLACE INTO settings (key, value) VALUES ('smtp_pass', ?)").run(smtp_pass);
    if (admin_email !== undefined) db.prepare("INSERT OR REPLACE INTO settings (key, value) VALUES ('admin_email', ?)").run(admin_email);
  ...
  app.post("/api/ai/export-docx", async (req, res) => {
    ...
    const { Document, Packer, Paragraph, HeadingLevel, TextRun, AlignmentType, BorderStyle } = await import("docx");
  ...
  app.post("/api/community/plans/share", (req, res) => {
    const { userId, title, subject, classe, content } = req.body;
    try {
      db.prepare("INSERT INTO community_plans (user_id, title, subject, classe, content) VALUES (?, ?, ?, ?, ?)").run(
        userId, title, subject, classe, content
      );
      res.json({ success: true });
    } catch (e) {
      res.status(500).json({ error: "Erro ao compartilhar plano" });
    }
  });

  app.post("/api/admin/community/approve", (req, res) => {
  ...
  app.get("/api/chat/messages", (req, res) => {
    const messages = db.prepare(`
      SELECT cm.*, u.professor_nome, u.foto_url 
  ...
  // Enhanced Classroom Management Routes
  app.get("/api/teacher-classes/:userId", (req, res) => {
    const classes = db.prepare("SELECT * FROM teacher_classes WHERE user_id = ?").all(req.params.userId);
    res.json(classes);
  });

  app.post("/api/teacher-classes/add", (req, res) => {
    const { userId, name, classe, subject, period } = req.body;
    try {
      db.prepare("INSERT INTO teacher_classes (user_id, name, classe, subject, period) VALUES (?, ?, ?, ?, ?)").run(
        userId, name, classe, subject, period
      );
      res.json({ success: true });
    } catch (e) {
  ...
  app.delete("/api/teacher-classes/:id", (req, res) => {
    db.prepare("DELETE FROM teacher_classes WHERE id = ?").run(req.params.id);
    res.json({ success: true });
  });
