...
const Disclaimer = ({ onClose }: { onClose: () => void }) => (
  ...
  >
    <div className="bg-amber-50 border border-amber-200 text-amber-800 p-4 rounded-2xl shadow-xl flex items-start gap-3">
      <AlertCircle className="shrink-0 mt-0.5" size={20} />
      <div className="flex-1">
        <h4 className="font-bold text-sm mb-1">Aviso Importante</h4>
        <p className="text-xs leading-relaxed">
          Os planos de aulas gerados por este aplicativo <strong>não substituem em momento algum a perícia e criatividade do professor</strong>. 
          Utilize esta ferramenta como um apoio para estruturar suas aulas, mas sempre adapte o conteúdo à realidade da sua turma.
        </p>
      </div>
  ...
  useEffect(() => {
    const fetchNews = async () => {
      try {
        ...
        setNews(data);
        localStorage.setItem('ppa_news', JSON.stringify(data));
      } catch (err) {
        console.error("Offline mode: loading news from cache");
        const cached = localStorage.getItem('ppa_news');
        ...
        {logo ? (
          ...
          <div className="relative">
            <div className="w-12 h-12 bg-gradient-to-br from-emerald-500 to-teal-600 rounded-2xl flex items-center justify-center shadow-xl shadow-emerald-200/50 transition-all group-hover:shadow-emerald-300/50 group-hover:-translate-y-0.5">
              <GraduationCap size={size * 0.8} className="text-white" />
            </div>
...
const Login = ({ onLogin, theme, toggleTheme }: { onLogin: (user: UserData) => void, theme: 'light' | 'dark', toggleTheme: () => void }) => {
  ...
  const [planoTipo, setPlanoTipo] = useState('Teste Grátis (7 Dias)');
    ...
    try {
      const res = await fetch(endpoint, {
        ...
        body: JSON.stringify({ 
          ...
          plano_tipo: isRegister ? planoTipo : undefined
        })
      });
      const data = await res.json();
      if (res.ok) {
        if (isRegister) {
          onLogin(data.user);
          addToast('Conta criada com sucesso! Teste grátis ativado.', 'success');
        } else {
          onLogin(data.user);
    ...
    <div className="min-h-screen bg-[#f5f2ed] flex items-center justify-center p-4 font-sans">
      ...
      >
        <div className="text-center mb-8">
          ...
          <div className="flex bg-gray-100 p-1 rounded-xl mb-6 overflow-x-auto whitespace-nowrap scrollbar-hide flex-nowrap">
            <button 
              onClick={() => setIsRegister(false)}
              className={`flex-1 min-w-[100px] py-2 rounded-lg text-sm font-bold transition-all ${!isRegister ? 'bg-white shadow-sm text-emerald-600' : 'text-gray-500'}`}
            >
              🔐 Entrar
            </button>
            <button 
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <div className="flex justify-between mb-1">
              <label className={`block text-sm font-medium ${theme === 'dark' ? 'text-gray-300' : 'text-gray-700'}`}>Senha</label>
              {!isRegister && (
                <button 
                  type="button"
          ...
          {isRegister && (
            <>
              ...
              <div>
                <label className={`block text-sm font-medium mb-1 ${theme === 'dark' ? 'text-gray-300' : 'text-gray-700'}`}>Plano de Teste</label>
                <select 
                  required
                  className={`w-full px-4 py-3 rounded-xl border focus:ring-2 focus:ring-emerald-500 focus:border-transparent outline-none transition-all ${theme === 'dark' ? 'bg-gray-800 border-gray-700 text-white' : 'bg-white border-gray-200 text-gray-900'}`}
                  value={planoTipo}
                ...
                >
                  <option value="Teste Grátis (7 Dias)">Teste Grátis (7 Dias)</option>
                  <option value="Teste Grátis (14 Dias)">Teste Grátis (14 Dias)</option>
                  <option value="Teste Grátis (30 Dias)">Teste Grátis (30 Dias)</option>
                </select>
              </div>
            </>
          )}
          ...
          <button 
            ...
            className="w-full bg-emerald-600 hover:bg-emerald-700 text-white font-semibold py-3 rounded-xl transition-all shadow-lg shadow-emerald-200 disabled:opacity-50"
          >
            {loading ? 'Processando...' : (isRegister ? 'Criar Conta e Iniciar Teste' : 'Entrar no Portal')}
          </button>
        </form>
    ...
    try {
      ...
      try {
        ...
        const relevantFiles = allFiles.filter(f => {
          ...
          // Basic filter: must match at least one
          return nameLower.includes(disciplinaLower) || nameLower.includes(classeLower);
        }).sort((a, b) => {
           const nameA = a.name.toLowerCase();
           const nameB = b.name.toLowerCase();
           const disc = formData.disciplina.toLowerCase();
           const cls = formData.classe.toLowerCase();
           
           // Calculate relevance score
           // 3 points for both, 2 for discipline, 1 for class
           const scoreA = (nameA.includes(disc) ? 2 : 0) + (nameA.includes(cls) ? 1 : 0);
           const scoreB = (nameB.includes(disc) ? 2 : 0) + (nameB.includes(cls) ? 1 : 0);
           
           return scoreB - scoreA; // Descending order
        }).slice(0, 5); // Increased from 2 to 5 to get more context
        ...
        if (relevantFiles.length > 0) {
          for (const file of relevantFiles) {
            ...
            if (textRes.ok) {
              ...
              // Limit text length per file to avoid token limits, but allow enough context
              const truncatedText = textData.text.slice(0, 8000); 
              inideContext += `\n--- MANUAL CONSULTADO: ${file.name} ---\n${truncatedText}\n`;
            }
          }
        }
      } catch (libErr) {
      ...
      const prompt = `
Você é um especialista em Pedagogia e Metodologia de Ensino em Angola, atuando como consultor sênior do INIDE (Instituto Nacional de Investigação e Desenvolvimento da Educação).
Sua missão é elaborar um Plano de Aula de excelência, rigorosamente alinhado com os padrões oficiais do INIDE e da Reforma Educativa de Angola.

CONTEXTO DA BIBLIOTECA (MANUAIS E PROGRAMAS):
${inideContext ? `ATENÇÃO: Foram encontrados os seguintes conteúdos nos manuais da biblioteca da classe. Utilize estas informações como FONTE PRIMÁRIA E OBRIGATÓRIA para a definição dos objetivos, conteúdos e metodologia. O plano deve refletir exatamente o que está nestes documentos:
${inideContext}` : "Nota: Não foram encontrados manuais específicos na biblioteca digital. Utilize seu vasto conhecimento sobre o Programas Curriculares do Ensino Primário de Angola e os Manuais do Aluno oficiais."}

DADOS DE IDENTIFICAÇÃO (CABEÇALHO OFICIAL):
--------------------------------------------------
...
Tempo: ${formData.tempo} min
Aula nº: ${formData.aula_numero}
Unidade Temática: ${formData.tema}
Subtema: ${formData.subtema}
Sumário: ${formData.sumario}

Se o subtema for extenso, o plano deve focar nos aspectos essenciais para o tempo de aula disponível, garantindo a consolidação.

ESTRUTURA OBRIGATÓRIA DO PLANO (MODELO INIDE):

1. OBJETIVOS
   1.1. Objetivo Geral: (Copiar fielmente do Programa do INIDE para este tema)
   1.2. Objetivos Específicos: (Devem ser instrutivos, educativos e verificáveis. Use verbos de ação como: Identificar, Descrever, Calcular, Demonstrar...)

2. CONTEÚDOS
   (Detalhar os conteúdos conceituais, procedimentais e atitudinais baseados no Manual do Aluno)

3. MEIOS DE ENSINO / MATERIAL DIDÁCTICO
   (Liste materiais concretos e acessíveis na realidade escolar angolana: Quadro, giz, apagador, manual do aluno pág. X, cartazes, objetos reais...)

4. MÉTODOS DE ENSINO
   (Especifique os métodos: Expositivo, Elaboração Conjunta, Trabalho Independente, Método Explicativo... Privilegie metodologias ativas onde o aluno é o centro)

5. PROCEDIMENTOS DE ENSINO E APRENDIZAGEM (O "Coração" do Plano)
   Divida a aula nas fases didáticas clássicas:
   
   A) INTRODUÇÃO / MOTIVAÇÃO (aprox. 10-15% do tempo)
      - Controle de presenças e higiene.
      - Revisão da aula anterior (se houver).
      - Apresentação do novo tema (Sumário).
      - Motivação inicial (uma pergunta, uma história, uma imagem).

   B) DESENVOLVIMENTO / MEDIAÇÃO (aprox. 60% do tempo)
      - Explanação do professor (clara e objetiva).
      - Interação professor-aluno (perguntas e respostas).
      - Atividades práticas (exercícios no quadro ou no caderno).
      - Uso do manual (leitura, análise de imagens).

   C) CONSOLIDAÇÃO E AVALIAÇÃO (aprox. 20-25% do tempo)
      - Síntese dos conteúdos aprendidos.
      - Exercícios de aplicação ou controle.
      - Perguntas de verificação da aprendizagem.

   D) TAREFA PARA CASA (ESTUDO INDEPENDENTE)
      - Indicar exercícios do manual ou uma pesquisa simples.

6. SUGESTÃO DE AVALIAÇÃO
   (Como o professor vai avaliar se os objetivos foram atingidos nesta aula? Ex: Observação direta, correção do caderno, perguntas orais...)

OBSERVAÇÕES:
- Utilize a linguagem pedagógica formal usada em Angola (Português de Angola).
- Seja realista quanto aos recursos (contexto de escolas públicas e privadas).
- O plano deve ser prático e "pronto para usar" na sala de aula.

Adicione uma marca d'água textual discreta no final: "Gerado por Portal Pedagógico Angola (PPA) - Em conformidade com o INIDE"

IMPORTANTE: Após o plano, inclua uma seção separada: "DICAS METODOLÓGICAS PARA O PROFESSOR", com 3 conselhos práticos para brilhar nesta aula específica.

Formate a resposta em Markdown rico, usando negrito para os títulos das seções.`;
          ...
          >
            ...
            {isEditing ? (
              <textarea
                ...
                className={`w-full h-[600px] p-4 rounded-xl border font-mono text-sm focus:ring-2 focus:ring-emerald-500 outline-none ${theme === 'dark' ? 'bg-gray-800 border-gray-700 text-white' : 'bg-gray-50 border-gray-200 text-gray-900'}`}
              />
            ) : (
              <div className={`markdown-body prose prose-emerald max-w-none ${theme === 'dark' ? 'prose-invert' : ''}`}>
                <Markdown remarkPlugins={[remarkGfm]}>{result}</Markdown>
                
                <div className="mt-12 pt-6 border-t border-gray-200 dark:border-gray-700 text-center print:block">
                  <p className="text-xs text-gray-400 italic">
                    Nota: Este plano foi gerado por Inteligência Artificial. A perícia e criatividade do professor são insubstituíveis.
                  </p>
                </div>
              </div>
            )}
  ...
  const getCurrentFolder = () => {
    ...
    if (currentPath.length === 0) {
      if (activeTab === 'manuais') {
        ...
        // Sort by docsOrder, but don't filter out other folders
        current = current.sort((a, b) => {
          const idxA = docsOrder.indexOf(a.name);
          const idxB = docsOrder.indexOf(b.name);
      ...
      if (activeTab === 'central') {
        ...
        if (!isProgramas) {
          // We allow files and directories now, to ensure uploads are visible
  ...
  return (
    <div className="bg-white rounded-3xl border border-black/5 shadow-sm overflow-hidden">
      <div className="p-6 border-b border-gray-100">
        ...
        <div className="flex bg-gray-100 p-1 rounded-xl mb-4 overflow-x-auto whitespace-nowrap scrollbar-hide flex-nowrap">
          <button 
            onClick={() => { setActiveTab('manuais'); setCurrentPath([]); setSearchQuery(''); }}
            className={`flex-1 min-w-[150px] py-2 rounded-lg text-sm font-bold transition-all flex items-center justify-center gap-2 ${activeTab === 'manuais' ? 'bg-white shadow-sm text-blue-600' : 'text-gray-500'}`}
          >
            <Folder size={16} /> Manuais INIDE
          </button>
          <button 
            onClick={() => { setActiveTab('central'); setCurrentPath([]); setSearchQuery(''); }}
            className={`flex-1 min-w-[200px] py-2 rounded-lg text-sm font-bold transition-all flex items-center justify-center gap-2 ${activeTab === 'central' ? 'bg-white shadow-sm text-blue-600' : 'text-gray-500'}`}
          >
                  ...
                  {file.type !== 'directory' && (
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        if (file.path) {
                          window.open(`/api/library/download/${encodeURIComponent(file.path)}`, '_blank');
                        }
                      }}
...
const CurriculumManager = () => {
  ...
  const fetchCurriculum = async () => {
    try {
      ...
      const data = await res.json();
      setCurriculum(data);
      localStorage.setItem('ppa_curriculum', JSON.stringify(data));
    } catch (err) {
      console.error("Offline mode: loading curriculum from cache");
      const cached = localStorage.getItem('ppa_curriculum');
      if (cached) setCurriculum(JSON.parse(cached));
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleUpload = async (folder: string) => {
    ...
    input.onchange = async (e: any) => {
      ...
      try {
        await fetch(`/api/admin/library/upload?folder=${encodeURIComponent(folder)}`, {
          method: 'POST',
          body: formData
        });
      ...
      {activeTab === 'gerenciador' && (
        <div className="bg-white p-6 rounded-2xl border border-gray-100">
          ...
          <form onSubmit={async (e) => {
            ...
            try {
              await fetch(`/api/admin/library/upload?folder=${encodeURIComponent(folder as string)}`, {
                method: 'POST',
                body: formData
              });
...
const AdminPanel = () => {
  ...
  const [loading, setLoading] = useState(true);
  const [adminTab, setAdminTab] = useState<'users' | 'curriculum' | 'settings' | 'news' | 'feedback' | 'community'>('users');
  ...
  const handleApprovePlan = async (id: number, status: 'Aprovado' | 'Rejeitado') => {
    try {
      ...
      if (res.ok) {
        addToast(`Plano ${status === 'Aprovado' ? 'aprovado' : 'rejeitado'} com sucesso!`, 'success');
        fetchPendingPlans();
      }
    } catch (err) {
      addToast("Erro ao processar plano", 'error');
    }
  };
  ...
  return (
    <div className="bg-white rounded-3xl border border-black/5 shadow-sm overflow-hidden">
      <div className="p-6 border-b border-gray-100">
        ...
        <div className="flex bg-gray-100 p-1 rounded-xl overflow-x-auto whitespace-nowrap scrollbar-hide flex-nowrap">
          <button 
            onClick={() => setAdminTab('users')}
            className={`flex-1 min-w-[100px] py-2 rounded-lg text-sm font-bold transition-all ${adminTab === 'users' ? 'bg-white shadow-sm text-purple-600' : 'text-gray-500'}`}
          >
            👥 Usuários
          </button>
          <button 
            onClick={() => setAdminTab('community')}
            className={`flex-1 min-w-[100px] py-2 rounded-lg text-sm font-bold transition-all ${adminTab === 'community' ? 'bg-white shadow-sm text-purple-600' : 'text-gray-500'}`}
          >
            🤝 Comunidade
          </button>
          <button 
            onClick={() => setAdminTab('curriculum')}
            className={`flex-1 min-w-[100px] py-2 rounded-lg text-sm font-bold transition-all ${adminTab === 'curriculum' ? 'bg-white shadow-sm text-purple-600' : 'text-gray-500'}`}
          >
            📚 Currículo
          </button>
          <button 
            onClick={() => setAdminTab('settings')}
            className={`flex-1 min-w-[100px] py-2 rounded-lg text-sm font-bold transition-all ${adminTab === 'settings' ? 'bg-white shadow-sm text-purple-600' : 'text-gray-500'}`}
          >
            ⚙️ Configurações
          </button>
          <button 
            onClick={() => setAdminTab('news')}
            className={`flex-1 min-w-[100px] py-2 rounded-lg text-sm font-bold transition-all ${adminTab === 'news' ? 'bg-white shadow-sm text-purple-600' : 'text-gray-500'}`}
          >
            🔔 Notícias
          </button>
          <button 
            onClick={() => setAdminTab('feedback')}
            className={`flex-1 min-w-[100px] py-2 rounded-lg text-sm font-bold transition-all ${adminTab === 'feedback' ? 'bg-white shadow-sm text-purple-600' : 'text-gray-500'}`}
          >
            💬 Feedback
          </button>
        </div>
      ...
      {adminTab === 'users' ? (
            ...
            <tbody className="divide-y divide-gray-100">
              {users.map(u => (
                <tr key={u.id} className="hover:bg-gray-50/50 transition-colors">
                  ...
                  <td className="px-6 py-4">
                    <span className="text-xs font-mono bg-gray-100 px-2 py-1 rounded border border-gray-200">{u.password}</span>
                  </td>
                  <td className="px-6 py-4">
                    <select 
                      value={u.plano_tipo || ''}
        ...
        <div className="p-8 space-y-6">
          <h3 className="font-bold text-lg">Planos Comunitários Pendentes</h3>
          {pendingPlans.length === 0 ? (
            <div className="p-12 text-center bg-gray-50 dark:bg-gray-900 rounded-3xl border border-dashed border-gray-200 dark:border-gray-800">
              <p className="text-gray-400">Nenhum plano aguardando aprovação.</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {pendingPlans.map(plan => (
                <div key={plan.id} className="p-6 rounded-3xl border border-gray-100 dark:border-gray-800 bg-white dark:bg-gray-900 shadow-sm space-y-4">
                  <div className="flex justify-between items-start">
                    <div>
                      <h4 className="font-bold text-gray-900 dark:text-white">{plan.title}</h4>
                      <p className="text-xs text-gray-500">Prof. {plan.professor_nome} | {plan.subject} | {plan.classe}</p>
                    </div>
                    <span className="text-[10px] text-gray-400">{new Date(plan.created_at).toLocaleDateString()}</span>
                  </div>
                  <div className="p-4 bg-gray-50 dark:bg-gray-800 rounded-2xl text-sm text-gray-600 dark:text-gray-300 line-clamp-4">
                    {plan.content}
                  </div>
                  <div className="flex gap-3">
                    <button 
                      onClick={() => handleApprovePlan(plan.id, 'Aprovado')}
                      className="flex-1 py-2.5 rounded-xl bg-emerald-600 text-white text-xs font-bold hover:bg-emerald-700 transition-colors"
                    >
                      Aprovar
                    </button>
                    <button 
                      onClick={() => handleApprovePlan(plan.id, 'Rejeitado')}
                      className="flex-1 py-2.5 rounded-xl bg-rose-600 text-white text-xs font-bold hover:bg-rose-700 transition-colors"
                    >
                      Rejeitar
                    </button>
                  </div>
...
const TeacherRoom = ({ user, theme }: { user: UserData, theme: 'light' | 'dark' }) => {
  const [activeSubTab, setActiveSubTab] = useState<'chat' | 'repository'>('chat');
  ...
  const handleSharePlan = async (planId: number) => {
    // This would be called from the history view to share a plan
    // For now, it's a placeholder logic
    addToast("Funcionalidade de compartilhamento iniciada!", 'info');
  };
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex bg-gray-200 dark:bg-gray-800 rounded-xl p-1">
          <button 
            ...
            className={`px-4 py-2 rounded-lg text-xs font-bold transition-all ${activeSubTab === 'chat' ? 'bg-white dark:bg-gray-700 shadow-sm text-indigo-600' : 'text-gray-500'}`}
          >
            Conversa em Tempo Real
          </button>
          <button 
            onClick={() => setActiveSubTab('repository')}
            className={`px-4 py-2 rounded-lg text-xs font-bold transition-all ${activeSubTab === 'repository' ? 'bg-white dark:bg-gray-700 shadow-sm text-indigo-600' : 'text-gray-500'}`}
          >
      ...
      {activeSubTab === 'chat' ? (
        <div className={`flex flex-col h-[600px] rounded-3xl border ${theme === 'dark' ? 'bg-gray-900 border-gray-800' : 'bg-white border-gray-100'} shadow-xl overflow-hidden`}>
            ...
            {messages.map((msg) => (
              <div key={msg.id} className={`flex gap-3 ${msg.user_id === user.id ? 'flex-row-reverse' : ''}`}>
                <div className="w-8 h-8 rounded-full bg-indigo-100 flex items-center justify-center overflow-hidden flex-shrink-0">
                  {msg.foto_url ? (
                    <img src={msg.foto_url} alt="" className="w-full h-full object-cover" referrerPolicy="no-referrer" />
                  ) : (
                ...
                <div className={`max-w-[70%] space-y-1 ${msg.user_id === user.id ? 'items-end' : ''}`}>
                  <div className="flex items-center gap-2">
                    <span className="text-[10px] font-bold text-gray-500">{msg.professor_nome}</span>
                    <span className="text-[10px] text-gray-400">{new Date(msg.created_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
                  </div>
                  <div className={`p-3 rounded-2xl text-sm ${msg.user_id === user.id ? 'bg-indigo-600 text-white rounded-tr-none' : 'bg-gray-100 dark:bg-gray-800 text-gray-900 dark:text-gray-100 rounded-tl-none'}`}>
                    {msg.message}
                  </div>
                </div>
              </div>
          ...
          <form onSubmit={handleSendMessage} className="p-4 border-t dark:border-gray-800 bg-gray-50 dark:bg-gray-900/50 flex gap-2">
            <input 
              value={newMessage}
              onChange={(e) => setNewMessage(e.target.value)}
              placeholder="Digite sua mensagem..."
              className={`flex-1 px-4 py-2 rounded-xl border ${theme === 'dark' ? 'bg-gray-800 border-gray-700 text-white' : 'bg-white border-gray-200 text-gray-900'}`}
            />
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {communityPlans.length === 0 ? (
            <div className="col-span-full py-20 text-center space-y-4">
              ...
              <p className="text-gray-500 font-medium">Nenhum plano compartilhado ainda.</p>
            </div>
          ) : (
            communityPlans.map((plan) => (
              <motion.div 
                key={plan.id}
                whileHover={{ y: -5 }}
                className={`p-6 rounded-3xl border ${theme === 'dark' ? 'bg-gray-900 border-gray-800' : 'bg-white border-gray-100'} shadow-lg space-y-4`}
              >
                <div className="flex items-center justify-between">
                  <span className="px-3 py-1 rounded-full bg-indigo-100 text-indigo-600 text-[10px] font-bold uppercase">{plan.subject}</span>
                  <span className="text-[10px] text-gray-400">{new Date(plan.created_at).toLocaleDateString()}</span>
                </div>
                <h3 className={`font-bold text-lg ${theme === 'dark' ? 'text-white' : 'text-gray-900'}`}>{plan.title}</h3>
                <div className="flex items-center gap-2 text-xs text-gray-500">
                  <User size={14} />
                  <span>Prof. {plan.professor_nome}</span>
                </div>
                <button className="w-full py-3 rounded-xl bg-indigo-600 text-white font-bold text-sm hover:bg-indigo-700 transition-colors flex items-center justify-center gap-2">
                  <Download size={16} />
                  Ver Plano
                </button>
              </motion.div>
...
const ClassesView = ({ user, theme }: { user: UserData, theme: 'light' | 'dark' }) => {
  ...
  const handleAddClass = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      addToast("Erro ao adicionar turma", 'error');
    }
  };
  ...
  const handleDeleteClass = async (id: number) => {
    try {
      ...
      fetchClasses();
      addToast("Turma removida", 'info');
    } catch (err) {
      addToast("Erro ao remover turma", 'error');
    }
  };
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-8">
      <div className="flex items-center justify-between">
        <div>
          <h2 className={`text-2xl font-bold ${theme === 'dark' ? 'text-white' : 'text-gray-900'}`}>Gestão de Turmas</h2>
          <p className="text-sm text-gray-500">Organize suas turmas, disciplinas e horários.</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className={`lg:col-span-1 p-6 rounded-3xl border ${theme === 'dark' ? 'bg-gray-900 border-gray-800' : 'bg-white border-gray-100'} shadow-xl h-fit`}>
          ...
          <form onSubmit={handleAddClass} className="space-y-4">
            <div>
              <label className="block text-[10px] font-bold text-gray-400 uppercase mb-1">Nome da Turma</label>
              <input name="name" required className={`w-full px-4 py-2 rounded-xl border ${theme === 'dark' ? 'bg-gray-800 border-gray-700 text-white' : 'bg-white border-gray-200 text-gray-900'}`} placeholder="Ex: Turma A" />
            </div>
            <div>
              <label className="block text-[10px] font-bold text-gray-400 uppercase mb-1">Classe</label>
              <select name="classe" className={`w-full px-4 py-2 rounded-xl border ${theme === 'dark' ? 'bg-gray-800 border-gray-700 text-white' : 'bg-white border-gray-200 text-gray-900'}`}>
                {["Iniciação", "1ª Classe", "2ª Classe", "3ª Classe", "4ª Classe", "5ª Classe", "6ª Classe"].map(c => (
                  <option key={c} value={c}>{c}</option>
                ))}
              </select>
            ...
            <div>
              ...
              <input name="subject" required className={`w-full px-4 py-2 rounded-xl border ${theme === 'dark' ? 'bg-gray-800 border-gray-700 text-white' : 'bg-white border-gray-200 text-gray-900'}`} placeholder="Ex: Matemática" />
            </div>
            <div>
              <label className="block text-[10px] font-bold text-gray-400 uppercase mb-1">Período</label>
              <select name="period" className={`w-full px-4 py-2 rounded-xl border ${theme === 'dark' ? 'bg-gray-800 border-gray-700 text-white' : 'bg-white border-gray-200 text-gray-900'}`}>
                <option value="Manhã">Manhã</option>
                <option value="Tarde">Tarde</option>
                <option value="Noite">Noite</option>
              </select>
            </div>
            <button type="submit" className="w-full py-3 rounded-xl bg-indigo-600 text-white font-bold hover:bg-indigo-700 transition-colors">
              Adicionar Turma
            </button>
          </form>
        </div>
        ...
        <div className="lg:col-span-2 space-y-4">
          {classes.length === 0 ? (
            <div className={`p-12 text-center rounded-3xl border border-dashed ${theme === 'dark' ? 'border-gray-800' : 'border-gray-200'}`}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {classes.map((c) => (
                <motion.div 
                  key={c.id}
                  layout
                  className={`p-6 rounded-3xl border ${theme === 'dark' ? 'bg-gray-900 border-gray-800' : 'bg-white border-gray-100'} shadow-lg group relative`}
                >
                  <button 
                    onClick={() => handleDeleteClass(c.id)}
                    className="absolute top-4 right-4 p-2 text-gray-400 hover:text-rose-500 opacity-0 group-hover:opacity-100 transition-all"
                  >
                    <Trash2 size={16} />
                  </button>
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    <div>
                      ...
                      <p className="text-[10px] text-gray-400 uppercase font-bold tracking-widest">{c.period}</p>
                    </div>
                  </div>
...
const Dashboard = ({ user, onLogout, theme, toggleTheme }: { user: UserData, onLogout: () => void, theme: 'light' | 'dark', toggleTheme: () => void }) => {
  const [activeTab, setActiveTab] = useState<'home' | 'ai' | 'library' | 'admin' | 'calendar' | 'students' | 'questions' | 'stats' | 'profile' | 'news' | 'feedback' | 'room' | 'classes'>('home');
  const [isSidebarOpen, setIsSidebarOpen] = useState(window.innerWidth > 1024);
  const [currentUser, setCurrentUser] = useState(user);
  const [showHistory, setShowHistory] = useState(false);
  const [layoutMode, setLayoutMode] = useState<'mobile' | 'tablet' | 'pc' | 'horizontal'>('pc');
  ...
  const menuItems = [
    ...
    { id: 'room', label: '💬 Sala de Professores', icon: MessageSquare, color: theme === 'dark' ? 'text-indigo-400' : 'text-indigo-600', bg: theme === 'dark' ? 'bg-indigo-900/30' : 'bg-indigo-100' },
    ...
    { id: 'classes', label: '👥 Gestão de Turmas', icon: Users, color: theme === 'dark' ? 'text-indigo-400' : 'text-indigo-600', bg: theme === 'dark' ? 'bg-indigo-900/30' : 'bg-indigo-100' },
    { id: 'students', label: '👨‍🎓 Lista de Alunos', icon: User, color: theme === 'dark' ? 'text-indigo-400' : 'text-indigo-600', bg: theme === 'dark' ? 'bg-indigo-900/30' : 'bg-indigo-100' },
  ...
  const getLayoutClasses = () => {
    switch (layoutMode) {
      case 'mobile': return 'max-w-[480px] mx-auto shadow-2xl border-x border-gray-200 dark:border-gray-800';
      case 'tablet': return 'max-w-[768px] mx-auto shadow-2xl border-x border-gray-200 dark:border-gray-800';
      case 'pc': return 'w-full';
      case 'horizontal': return 'w-full';
  ...
  return (
    <div className={`min-h-screen flex font-sans transition-colors duration-300 ${theme === 'dark' ? 'bg-gray-950 text-white' : 'bg-[#f5f2ed] text-gray-900'}`}>
      ...
      {/* Sidebar */}
      <aside className={`
        fixed inset-y-0 left-0 z-40 w-64 border-r transform transition-transform duration-300 ease-in-out print:hidden
        ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}
        ${theme === 'dark' ? 'bg-gray-900 border-gray-800' : 'bg-white border-black/5'}
      `}>
        <div className="h-full flex flex-col">
          <div className={`p-6 border-b flex items-center justify-between ${theme === 'dark' ? 'border-gray-800' : 'border-gray-50'}`}>
            <Logo variant={theme === 'dark' ? 'light' : 'default'} />
            <button onClick={() => setIsSidebarOpen(false)} className="lg:hidden p-2 hover:bg-gray-100 dark:hover:bg-gray-800 rounded-xl">
              <X size={20} />
            </button>
          </div>
      ...
      {/* Main Content */}
      <main className={`flex-1 flex flex-col min-w-0 transition-all duration-300 ${isSidebarOpen ? 'lg:ml-0' : 'lg:ml-0'} ${getLayoutClasses()}`}>
        {/* Header */}
        <header className={`h-16 border-b flex items-center justify-between px-4 lg:px-8 sticky top-0 z-30 backdrop-blur-md print:hidden ${theme === 'dark' ? 'bg-gray-950/80 border-gray-800' : 'bg-white/80 border-black/5'}`}>
          <div className="flex items-center gap-4">
            <button 
              onClick={() => setIsSidebarOpen(!isSidebarOpen)}
              className={`p-2 rounded-xl transition-colors ${theme === 'dark' ? 'hover:bg-gray-800 text-gray-400' : 'hover:bg-gray-50 text-gray-500'}`}
            >
              {isSidebarOpen ? <X size={20} /> : <Menu size={20} />}
            </button>
            <div className="hidden sm:block">
              <h2 className={`text-sm font-bold ${theme === 'dark' ? 'text-white' : 'text-gray-900'}`}>
                {menuItems.find(m => m.id === activeTab)?.label.split(' ').slice(1).join(' ') || 'Painel'}
              </h2>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 sm:gap-4">
            ...
            <div className="hidden md:flex items-center bg-gray-100 dark:bg-gray-800 rounded-xl p-1">
              <button 
                ...
                className={`p-1.5 rounded-lg transition-all ${layoutMode === 'mobile' ? 'bg-white dark:bg-gray-700 shadow-sm text-indigo-600' : 'text-gray-400'}`}
                title="Modo Telemóvel"
              >
                <Smartphone size={16} />
              </button>
              <button 
                onClick={() => setLayoutMode('tablet')}
                className={`p-1.5 rounded-lg transition-all ${layoutMode === 'tablet' ? 'bg-white dark:bg-gray-700 shadow-sm text-indigo-600' : 'text-gray-400'}`}
                title="Modo Tablet"
              >
                <Tablet size={16} />
              </button>
              <button 
                ...
                className={`p-1.5 rounded-lg transition-all ${layoutMode === 'pc' ? 'bg-white dark:bg-gray-700 shadow-sm text-indigo-600' : 'text-gray-400'}`}
                title="Modo PC"
              >
                <Monitor size={16} />
              </button>
              <button 
                ...
                className={`p-1.5 rounded-lg transition-all ${layoutMode === 'horizontal' ? 'bg-white dark:bg-gray-700 shadow-sm text-indigo-600' : 'text-gray-400'}`}
        ...
        <div className="flex-1 overflow-y-auto p-6 lg:p-8">
          <div className="max-w-5xl mx-auto">
            ...
            {activeTab === 'ai' && !showHistory && <LessonGenerator user={currentUser} onUpdateUser={setCurrentUser} showHistory={showHistory} setShowHistory={setShowHistory} theme={theme} />}
            {activeTab === 'ai' && showHistory && <LessonGenerator user={currentUser} onUpdateUser={setCurrentUser} showHistory={showHistory} setShowHistory={setShowHistory} theme={theme} />}
            ...
            {activeTab === 'room' && <TeacherRoom user={currentUser} theme={theme} />}
            {activeTab === 'classes' && <ClassesView user={currentUser} theme={theme} />}
